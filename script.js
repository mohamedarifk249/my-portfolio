...
(function initCustomCursor() {
    ...
    const ease = 0.18; // trailing speed
    ...
    const hoverSelectors = [
        'a', 'button', '.connect-btn', '.figma-btn', '.learn-more',
        '.carousel-nav', '.carousel-card', '.contact-method', '.submit-btn'
    ];
    const addHover = (el) => {
    ...
    document.querySelectorAll(hoverSelectors.join(',')).forEach(addHover);

    // Clean up on unload
    window.addEventListener('beforeunload', () => cancelAnimationFrame(rafId));
})();

// Interactive Flying Bug
(function initFlyingBug() {
    ...
    function updateBug() {
        ...
        if (cursorDistance < 80) {
            // Flee from cursor
            targetX = bugX - cursorDx * 2;
            targetY = bugY - cursorDy * 2;
            // Keep within bounds
        ...
        // Move bug
        bugX += (targetX - bugX) * 0.02;
        bugY += (targetY - bugY) * 0.02;
        
        // Add slight random movement
        bugX += (Math.random() - 0.5) * 2;
        bugY += (Math.random() - 0.5) * 2;
    ...
    function showBugCatchNotification() {
        const notification = document.createElement('div');
        notification.textContent = '🐛 Bug Caught! +1 Point';
        notification.style.cssText = `
            position: fixed;
            top: 2rem;
            left: 50%;
            ...
            box-shadow: 0 4px 20px rgba(75, 0, 0, 0.3);
        `;
